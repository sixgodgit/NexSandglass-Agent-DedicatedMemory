# NexSandglass 沙漏记忆系统⏳ V2.9.2「无限上下文」— 灵魂蒸馏 · 锚点引擎 · 越用越省
...
### V2.9.x「无限上下文」
```
V2.9.0: 待办直列5项(通用版) + Hermes内存关闭 + 灵魂注入重排(偏移→情绪→决策→纪律→待办) + 阶段链场景语义
V2.9.1: 锚点引擎30条 + ctx 500字 + 近期摘要替代对话原文 + 最近在做 + 最近决策注入
V2.9.2: 偏移优先重排 + per-turn对齐 + 英文Soul Distillation介绍恢复 + 近期摘要·最近在做·最近决策三大模块
```
